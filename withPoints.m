...
for k=1 : length(bboxes)
    ...
    corners = detectHarrisFeatures(int16(A));
    [features, valid_corners] = extractFeatures(int16(A), corners);
    
    Datacell(k,:)={A,size(A),B(2), valid_corners};
...
for k=1 : length(Excell_sorted)
    ...
    for j=1 : length(Datacell)
       ...
       letterFromDataBase = Datacell(j,4);
        letterFromDataBase = letterFromDataBase{1,1};
       
    corners = detectHarrisFeatures(int16(letterFromInput));
    [features, valid_corners] = extractFeatures(int16(letterFromInput), corners);
    
    numOfEqual=0;
    for a=1:length(valid_corners)
        if a<= length(letterFromDataBase) && valid_corners.Location(a,1) == letterFromDataBase.Location(a,1) && valid_corners.Location(a,2) == letterFromDataBase.Location(a,2)
            
            numOfEqual = numOfEqual + 1 ;
        end
    end
    
    if numOfEqual == length(valid_corners)
